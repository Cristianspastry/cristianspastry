...
import { ArrowRight, Clock, Wrench, Lightbulb, BookOpen } from 'lucide-react'
import { cn, formatDuration } from '@/core/utils'
import FavoriteButton from '@/features/favorites/components/FavoriteButton'
import type { Technique, TechniquePreview } from '../types'
          ...
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            {technique.difficulty && (
              <div className="absolute left-5 top-5">
                <span
                  className={cn(
                    'rounded-full px-4 py-2 text-sm font-semibold text-white shadow-md',
                    difficultyColors[technique.difficulty] ?? 'bg-gray-600'
                  )}
                >
            ...
            <div className="mb-4 flex flex-wrap items-center gap-4 text-sm text-gray-700">
              {technique.executionTime && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{formatDuration(technique.executionTime)}</span>
                </div>
              )}
