...
import type { KeyTakeaway } from '@/features/science/types'

interface ScienceKeyTakeawaysProps {
  keyTakeaways: KeyTakeaway[]
}

export default function ScienceKeyTakeaways({ keyTakeaways }: ScienceKeyTakeawaysProps) {
  return (
        ...
        {keyTakeaways.map((takeaway, index) => (
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-gray-700 leading-relaxed font-semibold">{takeaway.point}</p>
              {takeaway.explanation && (
                <p className="mt-2 text-sm text-gray-600">{takeaway.explanation}</p>
              )}
            </div>
          </div>
        ))}
